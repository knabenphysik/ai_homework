...
In the most common scenario, the classes are taken to be disjoint, so that **_each input is assigned to one and only one class_**. The input space is thereby divided into decision regions whose boundaries are called _**decision boundaries**_.
...
- k-means clustering: Classify into any number of classes, or regression

::: {.callout-tip}
Practical example with supervised learning.
:::
