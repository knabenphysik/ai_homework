...
Currently, the majority of AI applications are _**model-centric**_, one possible reason behind this is that the AI sector pays careful attention to academic research on models. According to Andrew Ng [@andrew], more than 90% of research papers in this domain are model-centric. In this approach, data is frequently overlooked, and data collection is viewed as a one-time event.

In _**model-centric**_ approach, you hold the model or code fixed and iteratively improve the quality of the data. This is shift from big data to good data.
...
_**data cleaning**_ is no longer merely a "pre-processing" step, but an iterative ML process!
